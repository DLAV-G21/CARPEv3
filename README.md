...
```
If the path to coco annotations is given (the annotation files corresponding to the imagesin path/to/images), the performances will be computed adn the Average precision at different level will be displayed. --viz allows to make the visualizations and save them in inference_out_folder. The option -j allows to save the output as a json file in the output folder.
